...
const ListOrdersCustomerDaGiao = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.post("http://localhost:3003/api/v1/donhangdagiaokhachhang",
                {
                    taikhoan: username
                }
            );
                ...
                <tbody>
                    {ListOdersChuaGiao.map((order, index) => (
                        <tr key={index}>
                            ...
                            <td><strong className="text-success">{order.trangthai}</strong></td>
                            <td>
...
export default ListOrdersCustomerDaGiao;
