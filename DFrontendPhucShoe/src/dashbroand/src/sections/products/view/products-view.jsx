...
export default function ProductsView() {
  ...
  const [DataShoes, setDataShoes] = useState({ data: [], loading: true, error: null });
  ...
  const fetchData = async () => {
    try {
      const response = await axiosWithCredentials.get("http://localhost:3003/api/v1/productall");
      setDataShoes({
  ...
  return (
    <>  <Container>
      ...
      <Grid container spacing={3}>
        {DataShoes.data.map((product) => (
          <Grid key={product.MASP} xs={12} sm={6} md={3}>
